...
if(empty($_SESSION)) {
  ...
  if(isset($_POST['email'])) {
    ...
    $stmt = $conn->prepare("SELECT * from tbl_staffs_a175054 WHERE fld_staff_email = :email");
   ...
   // $_SESSION['name'] = $readrow["fld_staff_name"];
    ...
    if ($count < 1) {
      ...
      $_SESSION['name'] = $readrow["fld_staff_name"];
      echo "<script>alert('Welcome {$_SESSION['name']}! You are logged in as {$_SESSION['role']}');document.location='main.php'</script>";
      if(!session_id()) 
        session_start();
    }
